...
@Component({
  selector: 'app-similar-bougth',
  templateUrl: './similar-bougth.component.html',
  styleUrls: ['./similar-bougth.component.css']
})
export class SimilarBougthComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._productsService.getFilterData("url", param).subscribe(resp => {
      ...
      for(let i in resp) {
        ...
        this._productsService.getFilterData("sub_category", resp[i].sub_category).subscribe(resp2 => {
